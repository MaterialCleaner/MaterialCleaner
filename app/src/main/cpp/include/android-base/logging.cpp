...
// Only used for Q fallback.
        ...
        void
        KernelLogger(android::base::LogId, android::base::LogSeverity severity, const char *tag,
...
#endif
            auto output_string =
                    StderrOutputGenerator(now, getpid(), GetThreadId(), severity, tag, file, line,
                                          message);
        ...
        static void
        LogdLogChunk(LogId id, LogSeverity severity, const char *tag, const char *message) {
...
//  }
        ...
        void
        LogdLogger::operator()(LogId id, LogSeverity severity, const char *tag, const char *file,
                               unsigned int line, const char *message) {
            if (id == DEFAULT) {
                id = default_log_id_;
            }
...
// checks/logging in a function.
        ...
        LogMessage::LogMessage(const char *file, unsigned int line, LogSeverity severity,
                               const char *tag,
                               int error)
...
//    }
        ...
        void LogMessage::LogLine(const char *file, unsigned int line, LogSeverity severity,
                                 const char *tag,
                                 const char *message) {
            int32_t priority = LogSeverityToPriority(severity);
//  if (__builtin_available(android 30, *)) {
