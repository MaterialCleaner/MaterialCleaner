 ...
 * SimpleListDiffer directly. This SimpleListDiffer can be used for complex cases, where overriding an
 * adapter base class to support asynchronous List diffing isn't convenient.
 * <p>
 * The SimpleListDiffer can consume the values from a LiveData of <code>List</code> and present the
 ...
 *     private final SimpleListDiffer&lt;User> mDiffer = new SimpleListDiffer(this, DIFF_CALLBACK);
 *     {@literal @}Override
 ...
 * }</pre>
 *
 * @param <T> Type of the lists this SimpleListDiffer will receive.
 * @see DiffUtil
...
public class SimpleListDiffer<T> {
    ...
    /**
     * Convenience for
     * {@code SimpleListDiffer(new AdapterListUpdateCallback(adapter),
     ...
     */
    public SimpleListDiffer(@NonNull RecyclerView.Adapter adapter,
    ...
    /**
     * Create a SimpleListDiffer with the provided config, and ListUpdateCallback to dispatch
     * updates to.
     *
    ...
    public SimpleListDiffer(@NonNull ListUpdateCallback listUpdateCallback,
    ...
    public void setCurrentList(@Nullable final List<T> newList) {
        setCurrentList(newList, null);
    }
